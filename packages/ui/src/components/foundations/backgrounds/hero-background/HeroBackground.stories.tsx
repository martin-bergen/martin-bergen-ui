...
const HERO_VIDEOS = [
  "hero-particles-01.mp4",
  "hero-particles-02.mp4",
];
...
export const WithImage: StoryObj<HeroBackgroundProps> = {
  args: {
    mediaType: "image",
    mediaSrc: `/backgrounds/images/${HERO_IMAGES[0]}`,
    particleCount: 80,
    particleColor: "moss",
    particleOpacity: 0.6,
    particleSize: 2,
  ...
  argTypes: {
    ...
    mediaSrc: {
      control: { type: "select" },
      options: HERO_IMAGES.map((img) => `/backgrounds/images/${img}`),
...
export const WithVideo: StoryObj<HeroBackgroundProps> = {
  args: {
    mediaType: "video",
    mediaSrc:
      `/backgrounds/videos/${HERO_VIDEOS[1]}`,
    fallbackImageSrc: `/backgrounds/images/${HERO_IMAGES[0]}`,
    particleCount: 100,
  ...
  argTypes: {
    ...
    mediaSrc: {
      control: { type: "select" },
      options: HERO_VIDEOS.map((video) => `/backgrounds/videos/${video}`),
...
export const MultipleLocalImages: StoryObj<HeroBackgroundProps> = {
  ...
  render: () => {
    ...
    const images = HERO_IMAGES.map((filename) => ({
      src: `/backgrounds/images/${filename}`,
      title: filename
        .replace(/\.[^/.]+$/, "")
        .replace(/_/g, " ")
        .replace(/-/g, " "),
    ...
    return (
      <div>
        ...
        <HeroBackground
          mediaType="image"
          mediaSrc={
            images[currentImage]?.src || `/backgrounds/images/${HERO_IMAGES[0]}`
          }
            ...
            <div className="space-y-4">
              {HERO_VIDEOS.map((filename, index) => (
                <div key={index}>
                  <p className="text-white/60 mb-2 text-xs">{filename}</p>
                  <video
                    src={`/backgrounds/videos/${filename}`}
                    controls
                    className="w-full max-w-2xl rounded-lg border-2 border-white/20"
                  />
                </div>
